...
function Upcıming() {
  ...
  const status = useAppSelector((state) => state.upcomingMovies.loading);
  const nextPage = useAppSelector((state) => state.upcomingMovies.page);
  ...
  return (
    <div className="bg-slate-900">
      <aside className="px-10 py-2">
        <h2 className="text-2xl font-semibold text-zinc-300">Gelecek Filmler</h2>
      </aside>
      ...
      <div className="flex  flex-col md:flex-row items-center justify-center px-6 py-4 space-y-2 text-center">
        <button
          type="button"
          onClick={() => dispatch(fetchUpcomingMovies(nextPage))}
