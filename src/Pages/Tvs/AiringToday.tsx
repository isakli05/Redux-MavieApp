...
import { fetchTvAirToday } from "../../redux/tvShows/tvAirTodaySlice";
import { Link } from "react-router-dom";

function AiringToday() {
  const tvOnAir = useAppSelector((state) => state.tvAirToday.data);
  const status = useAppSelector((state) => state.tvAirToday.loading);
  const nextPage = useAppSelector((state) => state.tvAirToday.page);
  const dispatch = useAppDispatch();
  useEffect(() => {
    if (status === "idle") {
      dispatch(fetchTvAirToday());
    }
  }, []);
  ...
  return (
    <div className="dark:bg-slate-900">
      <aside className="px-10 py-2">
        <h2 className="text-2xl font-semibold text-zinc-300">Bugün Yayınlanan TV Şovları
</h2>
      </aside>
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {tvOnAir?.results.map((tv) => (
          <div
            key={tv.id}
      ...
      <div className="flex  flex-col md:flex-row items-center justify-center px-6 py-4 space-y-2 text-center">
        <button
          type="button"
          onClick={() => dispatch(fetchTvAirToday(nextPage))}
